...
public interface GroupDao {
   ...
   // void insertGroup(Dbgroups group);
    ...
    @Query("SELECT dbgroups.id_group, dbgroups.`group`, dbgroups.use_group, sel.description from dbgroups LEFT JOIN (SELECT id_group , COUNT(*) description from dbgroupsandwords GROUP BY id_group) AS sel ON dbgroups.id_group = sel.id_group ")
    List<Dbgroups> getGroupsFiltered3();
